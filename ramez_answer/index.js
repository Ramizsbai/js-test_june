...
// #### 2. Validation 
// Come up with a new way to validate a pin code with a simple function,
// called validatePin that returns true (for a valid pin code) or false,
// (for an invalid pin code).
...
console.log(`-----------4----------`);
 ...
 let highestTurnout = events.sort((a, b) => (a.turnout < b.turnout) ? 1 : -1);

 console.table(highestTurnout);
...
console.log(`-----------5----------`);
 ...
 function squareOdds (arrNum){
     let square = arrNum.map(Math.sqrt);
     square.filter(num => {
        if(num % 2 !== 0){
            console.log(num)
        }});    
...
let now = new Date();
let before = new Date('July 13 2014');
...
const hourTracking = [
    ...
    let workingHours = hourTracking.map(hours => hours.end - hours.start);


    let total = workingHours.reduce((a, b) => a + b, 0);
...
for (let i = 0; i < board.length; i++) {
    ...
    let lengthArray = board[i].length;

    for (let l = 0; l < lengthArray; l++) {
        console.log('[' + i + ',' + l + '] = ' + board[i][l]);
    }};
